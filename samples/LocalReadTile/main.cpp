...
struct GData {
  float4 composed [[color(0)]];
  float4 albedo   [[color(1)]];
  float4 normal   [[color(2)]];
  float4 worldPos [[color(3)]];
};

kernel void lightingTile(imageblock<GData, imageblock_layout_implicit> ib, ushort2 tid [[thread_position_in_threadgroup]]) {
  GData px = ib.read(tid);
  const float3 albedo   = px.albedo.rgb;
  const float3 N        = normalize(px.normal.xyz * 2.0 - 1.0);
  const float3 worldPos = px.worldPos.xyz;
  const float3 L = normalize(float3(1.0, 1.0, -1.0) - worldPos);
  const float NdotL = clamp(dot(N, L), 0.3, 1.0);
  px.composed = float4(NdotL * albedo, 1.0);
  ib.write(px, tid);
}
)";
...
class LocalReadTile final : public lvk::metal::ISample {
 public:
  void init(lvk::metal::IMetalContext& ctx, GLFWwindow*, uint32_t width, uint32_t height, float, uint32_t) override {
    ...
    lvk::Holder<lvk::ShaderModuleHandle> deferredVert =
        ctx.createShaderModule({kShaderMSL, "deferredVertexMain", lvk::Stage_Vert, "deferred.vert"});
    lvk::Holder<lvk::ShaderModuleHandle> deferredFrag =
        ctx.createShaderModule({kShaderMSL, "deferredFragmentMain", lvk::Stage_Frag, "deferred.frag"});
    lvk::Holder<lvk::ShaderModuleHandle> tileFn = ctx.createShaderModule({kShaderMSL, "lightingTile", lvk::Stage_Comp, "lighting.tile"});

    const lvk::Format formats[4] = {
        ctx.getSwapchainFormat(),
        ctx.getFormat(gAlbedo_),
        ctx.getFormat(gNormal_),
        ctx.getFormat(gWorldPos_),
    };

    pipelineDeferred_ = ctx.createRenderPipeline({
        .smVert = deferredVert,
        .smFrag = deferredFrag,
        .color = {{.format = formats[0]}, {.format = formats[1]}, {.format = formats[2]}, {.format = formats[3]}},
        .depthFormat = lvk::Format_Z_F32,
        .debugName = "Pipeline: deferred",
    });
    pipelineTile_ = ctx.createTileRenderPipeline({
        .smTile = tileFn,
        .color = {formats[0], formats[1], formats[2], formats[3]},
        .debugName = "Pipeline: tile lighting",
    });
  }
  ...
  void render(lvk::ICommandBuffer& cmd, lvk::TextureHandle target, float time) override {
    ...
    cmd.cmdPushDebugGroupLabel("Tile lighting");
    lvk::metal::IMetalCommandBuffer& mcmd = static_cast<lvk::metal::IMetalCommandBuffer&>(cmd);
    mcmd.cmdBindTilePipeline(pipelineTile_);
    mcmd.cmdDispatchTile();
    cmd.cmdPopDebugGroupLabel();
 ...
 private:
  ...
  lvk::Holder<lvk::metal::TilePipelineHandle> pipelineTile_;
  uint32_t width_ = 0;
  uint32_t height_ = 0;
};

DESKTOP_MAIN(LocalReadTile)
