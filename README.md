...
- **Bindless is the one and only way** — global resource/sampler heaps of `MTL::ResourceID` plus a GPU-address buffer heap, addressed in shaders via `MTL4::ArgumentTable` at fixed slots; the bindless index is simply `handle.index()`. The function `cmdBindVertexBuffer()` is not implemented.
- **lvk-style command model** — `VulkanImmediateCommands`-equivalent ring of reusable `MTL4::CommandBuffer`s gated by a single `MTL::SharedEvent` timeline.
- **Native MSL shaders** — text via `newLibrary(source)` or precompiled `metallib` via `newLibrary(dispatch_data)`. Samples also demonstrate authoring shaders in Slang as an alternative to MSL.
