...
| **MachineLearning** — a tiny **NeRF** (neural radiance field) of the glTF Duck, rendered **GPU-driven** in one command buffer: a compute pass builds the network input from the live camera, `cmdDispatchNetwork` runs the MLP (`MTL4MachineLearningPipeline`), a second compute pass volume-composites, then a full-screen pass presents it — the duck rotates in real time. Model regenerated by `tools/ML/nerf.py` | <img src="tests/references/MachineLearning.png" width="320"> |
...
| Machine-learning pipeline (`MTL4MachineLearningPipeline`) | ✅ Implemented | Metal-only extension to run an ML network on the GPU timeline via `MTL4MachineLearningCommandEncoder`: `IMetalContext::createMachineLearningPipeline` (loads an `.mtlpackage` network, compiled by the context's `MTL4::Compiler`, with an auto-sized intermediates `MTL::Heap`) + `createTensor` (`MTL::Tensor`) + `setArgumentTableTensor` (`ArgumentKind::Tensors`) + `IMetalCommandBuffer::cmdBindMachineLearningPipeline` / `cmdDispatchNetwork`. See the `MachineLearning` sample (a tiny NeRF) |
