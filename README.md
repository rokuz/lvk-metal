...
| **MachineLearning** — a tiny **NeRF** of the glTF Duck, evaluated **GPU-driven** in one command buffer (compute → `cmdDispatchNetwork` → compute → present) and rotating in real time | <img src="tests/references/MachineLearning.png" width="320"> |
...
| **RTX_AO** — Bistro lit entirely by ray tracing in a compute kernel: a primary ray per pixel, then ray-traced ambient occlusion cached in a spatial hash (Gautron 2020) plus ray-traced shadows, with an ImGui control panel; a `Rendering` combo adds a ½-res + **CuNNy** ML upscale mode (the reusable `samples/common/upscaler`) | <img src="tests/references/RTX_AO.png" width="320"> |
| **RTX_IndirectTLAS** — a **GPU-built (indirect) TLAS**: each frame a compute shader writes native instance descriptors *and* a dynamic instance count into GPU buffers, then `cmdBuildIndirectTLAS` rebuilds the TLAS on the GPU timeline (no CPU translate, no readback) before an inline-RayQuery kernel traces the animated ring of icosahedra | <img src="tests/references/RTX_IndirectTLAS.png" width="320"> |
...
| Dynamic rendering (`cmdBeginRendering` / `cmdEndRendering`) | ✅ Implemented | Native to Metal render command encoders; LVK render-pass/framebuffer descriptors map directly. **Attachmentless** passes are supported — with no attachments to derive the render area from, `RenderPass::attachmentlessWidth`/`attachmentlessHeight` drive `renderTargetWidth`/`Height` (+ `defaultRasterSampleCount`), so a pipeline with no color/depth attachment can rasterize into buffers |
...
| Compressed formats (BC, ASTC, ETC2) | ✅ Implemented | BC1/BC3/BC4/BC5/BC7, ASTC 4x4/5x5/6x6 (LDR + sRGB) and ETC2 RGB8/sRGB8 map to their Metal pixel formats; `IContext::supportsTextureFormat()` reports what the device takes (BC is gated on `MTLDevice::supportsBCTextureCompression`) |
| Capability queries | ✅ Implemented | `supportsTextureFormat()`, `supportsShaderInterlock()` (Metal raster order groups, via `MTLDevice::rasterOrderGroupsSupported`), `supportsRayTracingPipeline()` = `false` (Metal has no SBT pipeline), `supportsAsyncCompute()` = `false` |
| MSAA + resolve | ✅ Implemented | Color and depth/stencil resolve |
| Depth & stencil state | ✅ Implemented | Dynamic depth + pipeline stencil combined into a cached `MTLDepthStencilState`; `cmdSetStencilRef`. **Stencil-only** attachments work too (`Format_S_UI8` → `MTLPixelFormatStencil8`): a depth-stencil attachment with no depth aspect is bound as a stencil attachment only |
...
| Slang → MSL | ✅ Implemented | Compiled at runtime (sample-side `SlangRuntime`). `samples/common/lvk_metal_bindless.slang` overloads `operator*` so matrices multiply GLSL-style, but since Slang 2026.16 a builtin-operator fast path rewrites `matrix * matrix` into a component-wise multiply *before* overload resolution; the documented carve-out is GLSL operator scope, so a shader that multiplies two matrices must start with `import glsl;` (the fast path then declines matrix operands and the overloads are used again). Scope it per shader, not globally via `-allow-glsl` — in GLSL scope other builtins resolve to the `glsl` module too, and some of those (`clamp`, mesh-stage entry points) are unavailable on the Metal target. `SlangRuntime` calls `compileBuiltinModule(BuiltinModuleName::GLSL)` because the `slang-glsl-module` dylib is not discoverable in-process |
...
| Ray tracing — acceleration structures, TLAS/BLAS, inline RayQuery | ✅ Implemented | BLAS/TLAS via `createAccelerationStructure` (MTL4 primitive/instance descriptors, built on a compute encoder); the TLAS is bindless (`ArgumentKind::AccelStructs`, indexed like a texture). Metal has no ray-tracing *pipeline* (no SBT / `DispatchRays` / raygen-miss-hit groups), so RT is expressed as **inline `metal::raytracing::intersector`** inside a compute or fragment shader; `cmdTraceRays` aliases `cmdDispatch`. The TLAS builds either CPU-driven (`cmdUpdateTLAS`) or **GPU-built / indirect** (`cmdBuildIndirectTLAS` reads compute-written instance descriptors + a dynamic count on the GPU timeline — no CPU translate). A BLAS created with `AccelStructBuildFlagBits_AllowUpdate` can be **refit** on the GPU with `cmdUpdateBLAS` (`refitAccelerationStructure`, scratch sized for the larger of build/refit); LVK build flags map to `MTLAccelerationStructureUsage` (`AllowUpdate`→`Refit`, `PreferFastTrace`→`PreferFastIntersection`, `PreferFastBuild`→`PreferFastBuild`, `LowMemory`→`MinimizeMemory`). See the `RTX_*` samples |
...
| Machine-learning pipeline (`MTL4MachineLearningPipeline`) | ✅ Implemented | Metal-only extension to run an ML network (an `.mtlpackage`) on the GPU timeline via `MTL4MachineLearningCommandEncoder`: `createTensor` + `createMachineLearningPipeline` (takes the input/output `TensorHandle`s) + `cmdBindMachineLearningPipeline` / `cmdDispatchNetwork`. See the `MachineLearning` (NeRF) sample, and the reusable `samples/common/upscaler` (CuNNy 2× super-resolution) wired into `RTX_AO` as a ½-res render + upscale mode |
